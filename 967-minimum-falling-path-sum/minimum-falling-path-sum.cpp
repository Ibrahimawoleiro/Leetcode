...
public:
    ...
    int optimizedTabulation(vector<vector<int>>& matrix){
        int row_size = matrix.size();
        int col_size = matrix[0].size();
        vector<int> prev(col_size, -1);
        vector<int> curr(col_size, -1);
        int ans;
        for (int r = row_size - 1; r >= 0; r--){
            ans = INT_MAX;
            for (int c = 0; c < col_size; c++){
                ...
                below = prev[c];
                curr[c] = matrix[r][c] + min(min(left, right), below);
                ans = min(ans, curr[c]);
            }
            prev = curr;
        }
    ...
    int minFallingPathSum(vector<vector<int>>& matrix) {
        return optimizedTabulation(matrix);
    }
};