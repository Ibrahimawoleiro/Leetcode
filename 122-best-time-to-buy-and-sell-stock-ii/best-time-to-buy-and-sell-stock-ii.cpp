...
public:
    ...
    //     }else{
    //         sell = max(prices[i] + recursive(prices, i + 1, false), recursive(prices, i + 1, true));
    //     }
    //     return max(buy, sell);
    // }
    ...
    //     }else{
    //         sell = max(prices[i] + memoized(prices, i + 1, false, dp), memoized(prices, i + 1, true, dp));
    //     }
    ...
    int tabulation(vector<int>& prices){
        ...
        for(int i = last_index - 1; i >= 0 ; i --){
                    ...
                    dp[i][bought] = max(prices[i] + dp[i + 1][not_bought], dp[i + 1][bought]);
        }
